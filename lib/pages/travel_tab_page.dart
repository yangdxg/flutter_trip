...
//继承AutomaticKeepAliveClientMixin重写wantKeepAlive，切换页面时不进行重绘
class _TravelTabPageState extends State<TravelTabPage>
  ...
  void _loadData({loadMore = false}) {
    ...
    TravelDao.fetch(
        widget.travelUrl, widget.groupChannelCode, pageIndex, PAGE_SIZE).then((
            ...
            child: MediaQuery.removePadding(
                ...
                child: RefreshIndicator(
                  ...
                  child: StaggeredGridView.countBuilder(
                    ...
                    //一个item占几列(和crossAxisCount配合使用)（4/2）（2/1）所以也可以填 2，1
      ...
      onTap: () {
        if (item.article.urls != null && item.article.urls.length > 0) {
          Navigator.push(context, MaterialPageRoute(builder: (contest) =>
              WebViewWidget(url: item.article.urls[0].h5Url, title: '详情',)));
        }
      },
      child: Card(
                ...
                children: <Widget>[
                  Padding(padding: EdgeInsets.only(left: 3),
                    child: Icon(Icons.location_on, color: Colors.white,
                      size: 12,),),
                  //控制组件的最大宽度和最大高度，控制显示单行文字，最后显示省略号
                  LimitedBox(
                    maxWidth: 130,
                    child: Text(_poiName(),
                      maxLines: 1,
